...
getConf_func(){
	if [[ $# != "5" && $# != "6" ]]; then 
		...
		# local TYPE=${5}
		local OUT_FILE=${5}
		local TARGET_BED=($(echo ${6} | sed 's/,/ /g'))
...
# Demultiplex by nanoBC 
##
## $1 = BCL directory
## $2 = Output directory
## $3 = SampleSheet (SampleSheet.csv obtained from the KDI)
## $4 = ref de séquence de Barcode (nano_BC_ref.csv)


demultiplex_nanobc_func()
{
  output_dir=$1
  index=$2
  forward=$3
  ...
  local log=$7/demultiplex_nanobc_func.log
  
  rm -rf ${output_dir}/fastqs/

  mkdir -p ${output_dir}/fastqs/
  #Create the file with the lines containing the barcodes (and the information lines)
  
  echo -e "Creating the files ${name}.lines_to_keep.txt"
  echo -e "Logs: $log"
  echo
  
  echo -e "Creating the files ${name}.lines_to_keep.txt" > ${log}
  echo >> ${log}
  
  gzip -cd ${index} | cut -c1-8 | grep -n -B 1 -A 2 --no-group-separator ${barcode_seq} | cut -d ':' -f 1 | cut -d '-' -f 1 > ${output_dir}/fastqs/${name}.lines_to_keep.txt

  echo -e "Running select_lines.sh to demultiplex fastqs"
  echo -e "Logs: $log"
  echo
  
  echo -e "Running select_lines.sh to demultiplex fastqs" > ${log}
  echo >> ${log}
...
remove_nanobc_read1N_func()
{
  ...
  local log=$4/remove_nanobc_read1N_func.log
  
  start_index_1=30
  size_index=16
  
  zcat ${index} | awk -v start_index_1=$start_index_1 -v size_index=$size_index  'NR%2==0{print substr($0,start_index_1,size_index)} ; NR%2==1{print}' | gzip > ${output_dir}/fastqs/${name}.R2.fastq.temp.gz
  mv ${output_dir}/fastqs/${name}.R2.fastq.temp.gz ${output_dir}/fastqs/${name}.R2.fastq.gz
...
{ 
	...
	echo -e "Concatenating Fastqs From 10X together..."
        ...
        # Changed because now I2 is R2 and R2 is R3 in the KDI 
...
if [[ ${read2} =~ \.gz ]]; then
         ...
         cmd="cat $read2 | awk -v start_index_1=$start_index_1 -v size_index=$size_index  'NR%4==1{print \">\"substr(\$1,2)}; NR%4==2{print substr(\$0, start_index_1,size_index)}' > ${out}/read_indexes_1.fasta"
